...
| 迁移 | 内容 | 旧应用兼容性 |
|---|---|---|
| 0004 | 新增 `refund_items`、`return_requests`、`return_request_items`、约束、索引与 RLS/授权 | 仅新增对象，旧应用可继续读写原订单级退款 |
...
1. 停止退款和售后写入，确认没有 `pending` 且仍持有处理租约的退款。
...
3. 回滚应用到不早于其数据库契约可兼容的版本。旧应用可以在 0004–0008 schema 上运行，因此若故障在应用层，优先只回滚应用并保留新表/新列。
