...
- 迁回境内时新建 PostgreSQL/对象存储与 Auth 实例，通过 `pg_dump` / `pg_restore` 迁移数据，切换 Storage URL 和环境变量，并在停机窗口重放迁移、核对库存/订单/支付及完整回归。具体见 [v0.1.0 技术方案](../archive/v0.1.0/TECH_SPEC.md#132-迁移国内方案) 与 [v0.3.0 迁移与回滚](../v0.3.0/MIGRATION-ROLLBACK-v0.3.0.md)。

## 历史部署记录（v0.2.1 预生产时期）

> 以下为 v0.2.1 预生产时期的部署约束与数据基线，保留供追溯；当前 stage/生产流程见本文上方各节，v0.2.3 上海低内存部署记录在 `release-v0.2.3` 分支。
