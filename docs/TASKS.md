...
- 已完成、待提交：T071 退款接口与后台操作页；Mock 回归已覆盖全额退款、库存回补、折扣释放、部分退款与超额退款拦截
- 支付宝沙箱配置与真实全链路回归按当前安排后续补充，不阻塞其余业务功能开发
- 下一步：T072 生产任务 → T073 生产看板 → T074 工作台 → T081 折扣码管理
...
执行顺序：T071 退款 → T072 生产任务 → T073 生产看板 → T074 工作台 → T081 折扣码管理；支付宝沙箱全链路回归在配置补齐后执行。
