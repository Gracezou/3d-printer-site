...
- 已完成代码与自动化验收：T060 Provider + Mock、T061 Alipay Provider、T062 支付接口、T063 异步回调、T064 支付页面
- 待沙箱全链路回归：将 `alipay-demo/.env.local` 中的应用 ID、私钥、公钥安全写入本项目 `.env.dev`，并配置本项目公网 HTTPS `ALIPAY_NOTIFY_URL` / `ALIPAY_RETURN_URL` 后完成一笔真实沙箱支付
- 支付回归通过后继续 T071～T074，再继续 T081
...
执行顺序：T060 Provider + Mock → T061 Alipay Provider → T062 支付接口 → T063 异步回调 → T064 支付页面 → 沙箱全链路回归。
